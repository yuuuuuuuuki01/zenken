...
ws.onmessage = (event) => {
    ...
    if (msg.type === 'init' || msg.type === 'update') {
        if (msg.payload.session && msg.payload.session.token) {
            ...
            const pt = document.getElementById('input-pairing-token');
            if (pt) pt.value = window.gigaToken;
        }
        if (msg.payload.serverUrl) {
            window.serverUrl = msg.payload.serverUrl;
            const su = document.getElementById('input-server-url');
            if (su) su.value = window.serverUrl;
        }
        if (msg.payload.nodeInfo && msg.payload.nodeInfo.name) {
            window.nodeName = msg.payload.nodeInfo.name;
            const nn = document.getElementById('input-node-name');
            if (nn) nn.value = window.nodeName;
        }
        ...
        if (statusBadge) {
            if (msg.payload.success) {
                ...
                statusBadge.textContent = msg.payload.error || '同期エラー';
                statusBadge.className = 'status-badge disconnected';
                if (msg.payload.details) console.warn(`[Sync] ${msg.payload.details}`);
            }
        }
...
ws.onclose = () => {
    const statusBadge = document.getElementById('connection-status');
    if (statusBadge) {
        statusBadge.textContent = 'エージェント未起動 (オフライン)';
        statusBadge.className = 'status-badge disconnected';
    }
};
...
ws.onerror = (err) => {
    ...
    const statusBadge = document.getElementById('connection-status');
    if (statusBadge) {
        statusBadge.textContent = 'バックグラウンド接続エラー';
        statusBadge.className = 'status-badge disconnected';
    }
};
...
if (saveSettingsBtn) {
    saveSettingsBtn.addEventListener('click', async () => {
        ...
        const nodeName = document.getElementById('input-node-name').value;
        ...
        const serverUrlInput = document.getElementById('input-server-url').value;
        const pairingTokenInput = document.getElementById('input-pairing-token').value;
        ...
        try {
            ...
            // Update Server URL locally
            if (serverUrlInput && serverUrlInput !== window.serverUrl) {
                ws.send(JSON.stringify({
                    type: 'update_settings',
                    payload: { serverUrl: serverUrlInput }
                }));
                requiresRestart = true;
            }

            // Update Pairing Token locally
            if (pairingTokenInput && pairingTokenInput !== window.gigaToken) {
                ws.send(JSON.stringify({
                    type: 'wizard_pair',
                    payload: { token: pairingTokenInput }
                }));
                requiresRestart = true;
            }
            ...
            ws.send(JSON.stringify({
                ...
                payload: {
                    ...
                    apiKeys: { openai: openaiKey, gemini: geminiKey }
                }
            }));


            // [Fix] Using full user token for authenticating save
            const targetServer = serverUrlInput || window.serverUrl || 'https://gigacompute-fleet.web.app';
            const apiUrl = targetServer.replace('ws://', 'http://').replace('wss://', 'https://');
            const usedToken = pairingTokenInput || window.gigaToken;

            if (usedToken && usedToken !== 'dummy-token') {
                const res = await fetch(`${apiUrl}/v1/worker/dashboard/profile`, {
                    method: 'PUT',
                ...
                if (res.ok) {
                    if (settingsMsg) {
                        settingsMsg.textContent = requiresRestart ? '設定を変更しました。新しい通信先へ再接続するためエージェントを再起動してください。' : 'クラウドと同期しました！';
                ...
                if (settingsMsg) {
                    settingsMsg.textContent = requiresRestart ? 'ローカル設定を保存しました（エージェント再起動が必要です）' : 'ローカル設定を保存しました';
                    settingsMsg.style.color = '#0f0';
                }
                setTimeout(() => window.location.href = '/index.html', 2500);
            }
            ...
            if (settingsMsg) {
                settingsMsg.textContent = 'ネットワークエラー（サーバーに接続できません）ローカル設定は保存されました。';
                settingsMsg.style.color = '#f00';
            }
        }
    });
