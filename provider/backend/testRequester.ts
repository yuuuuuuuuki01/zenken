...
process.env.DECRYPTION_KEY = crypto.randomBytes(32).toString('base64');

const ws = new WebSocket('wss://127.0.0.1:8081', { rejectUnauthorized: false });
...
ws.on('open', () => {
    ...
    setTimeout(async () => {
        try {
            ...
            const { encrypted, iv, authTag } = encrypt(Buffer.from(wasmCode), dummyKey);
