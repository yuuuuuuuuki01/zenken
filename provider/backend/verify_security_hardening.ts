...
async function verifyHardening() {
    ...
    const hackedFile = path.resolve(process.cwd(), 'HACKED_BY_POC.txt');
    ...
    try {
        ...
        const boundaryLeak = path.join(path.dirname(stagingDir), 'staging-secret/leak.txt');
        if (fs.existsSync(boundaryLeak)) {
