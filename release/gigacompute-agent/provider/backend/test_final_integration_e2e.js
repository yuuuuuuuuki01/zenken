...
// --- 1. Worker Setup ---
const workerKeys = (0, encryption_1.generateNodeKeypair)();
const workerWs = new ws_1.default(SERVER_URL, options);
workerWs.on('open', () => {
...
workerWs.on('message', (data) => {
    ...
    if (msg.type === 'task_request') {
        ...
        const decrypted = (0, encryption_1.decrypt)(encrypted, ENCRYPTION_KEY, iv, authTag);
        ...
        const signature = (0, encryption_1.signResult)(Buffer.from(JSON.stringify(result)), workerKeys.privateKey);
        const response = {
            taskId: task.taskId,
            status: 'success',
            result: result,
...
// --- 2. Requester Setup ---
const requesterWs = new ws_1.default(SERVER_URL, options);
requesterWs.on('open', () => {
    console.log('[Requester] Connected. Sending Encrypted Task...');
    const taskPayload = Buffer.from('Final PoC Wasm Payload Simulation');
    const { encrypted, iv, authTag } = (0, encryption_1.encrypt)(taskPayload, ENCRYPTION_KEY);
    const task = {
