...
function updateUI(data) {
    ...
    document.getElementById('ticker-amount').textContent = totalIncome.toFixed(6); // Set base text, ticker overwrites

    const roiCostEl = document.getElementById('roi-cost');
    const roiNetEl = document.getElementById('roi-net');
    const roiMonthlyEl = document.getElementById('roi-monthly');

    if (roiCostEl) roiCostEl.textContent = totalEstimatedCost.toFixed(4);
    if (roiNetEl) roiNetEl.textContent = netProfit.toFixed(6);
    if (roiMonthlyEl) roiMonthlyEl.textContent = monthlyForecast.toFixed(2);
    ...
    historyBody.innerHTML = localTasks.slice(0, 10).map(t => `
        <tr>
            ...
            <td>
                ${t.status === 'completed' ? `
                    <div style="font-size: 0.65rem; color: var(--accent-success);">+10.0</div>
...
function initResourceControls(nodeInfo) {
    ...
    // Hardware Specs (Simplified for demo, usually from nodeInfo)
    const maxCores = nodeInfo.totalCores || 16;
    const maxMem = nodeInfo.totalMemoryGb || 64;

    rangeCpu.max = maxCores;
    rangeMem.max = maxMem;

    // Logic-based Recommendation (Mathematical Algorithm)
    // Rule: Leave at least 2 cores and 4GB for OS/User unless ターボ mode
    const recCores = Math.max(1, maxCores - 2);
    const recMem = Math.max(2, Math.floor(maxMem * 0.7)); // 70% recommendation

    // Update Wizard Recommendations (New for Onboarding v2.0)
...
window.GigaUI = { update: updateUI, initialized: false };
