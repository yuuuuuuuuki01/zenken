...
const nodeInfo = {
    ...
    resourceLimits: {
        ...
        memoryGb: Math.floor(os_1.default.totalmem() / (1024 * 1024 * 1024) * 0.7)
    }
};
// 入札価格の初期算出
...
const guiServer = new guiServer_1.GuiServer({
    ...
    onCreateShortcut: async () => {
        ...
        if (isWin) {
            const shortcutPath = path_1.default.join(desktopPath, 'GigaComputeCockpit.lnk');
            console.log(`[GUI] Creating Windows App Mode shortcut at ${shortcutPath}`);
            const iconPath = path_1.default.join(baseDir, 'branding', 'icon.png');
            const psScript = `
...
$shortcut.Description = 'GigaCompute Agent Cockpit';
...
$shortcut.Save();
            ...
            require('child_process').exec(`powershell -command "${psScript.replace(/\n/g, '')}"`, (err) => {
                if (err)
                    ...
                    console.log("[GUI] App Mode shortcut created successfully on Desktop.");
            });
        }
        else if (isMac) {
            const shortcutPath = path_1.default.join(desktopPath, 'GigaComputeCockpit.command');
            console.log(`[GUI] Creating macOS launch script at ${shortcutPath}`);
            const scriptContent = `#!/bin/bash
open -a "Google Chrome" --args "--app=${targetUrl}" || open "${targetUrl}"
`;
            fs_1.default.writeFileSync(shortcutPath, scriptContent);
            fs_1.default.chmodSync(shortcutPath, '755');
            console.log("[GUI] macOS launch script created successfully on Desktop.");
        }
        else {
            console.log("[GUI] Shortcut creation not implemented for this OS yet.");
        }
    },
});
    ...
    if (isWin) {
        ...
        (async () => {
            ...
            if (!fs_1.default.existsSync(shortcutPath)) {
                ...
                // Reuse the existing shortcut creation logic helper if needed
            }
        })();
    }
    else if (isMac) {
