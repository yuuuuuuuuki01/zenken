...
const ws = new ws_1.default(SERVER_URL, agentOptions);
// Node Trust Protocol: キーペアの生成
const nodeKeys = (0, encryption_1.generateNodeKeypair)();
ws.on('open', () => {
...
ws.on('message', (data) => {
    ...
    if (msg.type === 'task_request') {
        ...
        const signature = (0, encryption_1.signResult)(Buffer.from(JSON.stringify(response.result)), nodeKeys.privateKey);
        ws.send(JSON.stringify({
            type: 'task_response',
            payload: {
                ...response,
