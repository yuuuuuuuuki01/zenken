    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === 'system_state') {
          ...
          setTasks((prev) => {
            const exists = prev.find((task) => task.taskId === payload.taskId);
            if (exists) {
              return prev.map((task) =>
                task.taskId === payload.taskId
                  ? {
                      ...task,
                      ...payload,
                      status: msg.type === 'task_response' ? 'completed' : 'pending',
                    }
                  : task,
              );
            }
            ...
            <div key={node.id} className="node-card glass-panel">
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                  <span className="node-spec-badge" title="Performance">
                    PERF {node.performanceScore}
                  </span>
                  <span className="node-reward-badge" title="Rewards">
                    PTS {node.rewardPoints}
                  </span>
      ...
      <main className="main-content">
        <div className="world-map">
          <svg viewBox="0 0 800 400" className="map-svg">
            ...
            <path
              d="M150,100 L200,80 L250,90 L300,120 L350,110 L400,130 L450,120 L500,140 L550,130 L600,150 L650,140 L700,160 L750,150 L800,170 L800,400 L0,400 L0,150 Z"
              opacity="0.1"
            ...
            {tasks
              ...
              .map((task) => {
                const requester = nodes.find((node) => node.id === task.requesterId);
                const worker = nodes.find((node) => node.id === task.workerId);
            ...
            {nodes.map((node) => {
              ...
              return (
                <g key={node.id} className="map-node">
                  ...
                  {node.status === 'busy' && <circle cx={x} cy={y} r="3" className="node-ripple" />}
                </g>
              );
            })}
          </svg>
        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task) => (
            <div key={task.taskId} className="task-item glass-panel">
              <div className="task-icon">JOB</div>
