...
function activate(context) {
    ...
    let disposableSubmit = vscode.commands.registerCommand('gigacompute.submitSelection', async () => {
        ...
        const serverUrl = config.get('serverUrl') || 'http://localhost:8081';
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                const response = await axios_1.default.post(`${serverUrl}/v1/client/task/submit`, {
                ...
                if (response.data.success) {
                    ...
                    outputChannel.show();
                    outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] Task Submited. Job ID: ${jobId}`);
                    ...
                    outputChannel.appendLine(`Waiting for distributed workers to complete... (Poling not yet implemented in PoC)`);
                }
                else {
